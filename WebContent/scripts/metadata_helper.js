...
function doSearch() {
	var theForm = document.forms["addform"];
	if(theForm) {
		var url = "metadata.search.PathSearchServlet?websiteid="+selectedWebsiteId+"&term=";
...
function selectSearchResult(id, theValue) {
	showOrHideDiv("searchResultContainer");
	var theForm = document.forms["addform"];
	if(theForm) {
		theForm.elements["pathid"].value = id;
//		theForm.elements["searchTerm"].value = theValue;
		theForm.elements["searchTerm"].style.display = "none";
		theForm.elements["suggested_url"].value = theValue;
		document.getElementById("pathPrefix").innerHTML += theValue;
		var theInfo = document.createElement("span");
		theInfo.className = "path";
		theInfo.innerHTML = theValue;
	}
	
	showOrHideDiv("submitButton");
}
				...
				if(count > 0) {
					...
					for(i = 0; i < listResults.length; i++) {
						...
						for(j = 0; j < listSites.length; j++) {
							...
							element2.innerHTML = "<a href='JavaScript: selectDomainSearchResult("+theSiteId+",\""+theSiteFullName+"\");'>"+theSiteFullName+"</a>";
...
function selectDomainSearchResult(theItem, theName) {
	var theForm = document.forms["addform"];
	if(theForm) {
		if(theItem > 0) {
			theForm.elements["website_search"].value = theName;
			theForm.elements["website_search"].disabled = true;
			theForm.elements["websiteid"].value = theItem;
			document.getElementById("pathPrefix").innerHTML += theName;
		}
		selectedWebsiteId = theItem;
		
		document.getElementById("domainsearch").innerHTML = "";
		
		//showOrHideDiv("searchTermBox");
		showOrHideDiv("searchTermSelectAgain");
		showOrHideDiv("websiteManualSelect");
		
		showOrHideDiv("pathDiv");
	}
}
...
function metaInformation() {
	showOrHideDiv("settings");
}

function showOrHideDiv(elementName) {
